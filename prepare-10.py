# Rough plan for preparing 10x10 files:
...
TORUS_SIZE = 10
cycle_frame_ids_filename = f'OrderedConfigurations{TORUS_SIZE}x{TORUS_SIZE}.txt'
...
def parse_evrd(line):
    evrd_values.extend([value.strip() for value in line.strip().split(',') if value != ''])
...
with open(cycle_frame_ids_filename) as cycle_frame_ids_file, \
    ...
    while True:
        ...
        parse_evrd(evrd_line)
        
        output_file.write(f'#{cycles_read + 1}: {','.join(special_params)},{evrd_values[cycles_read]}\n')
