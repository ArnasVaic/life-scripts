...
with open(input_filename) as input, \
    ...
    while True:
        ...
        config_line, eigenvalue_line, decays_line, rates_line = input.readline(), input.readline(), input.readline(), input.readline()
        ...
        # parse config line
        config_line_numbers = re.findall(r'\d+', config_line)
        config_line_numbers = [int(x) for x in config_line_numbers]
        # period = int(config_line_numbers[1])
        ...
        output.write(f'#{config_line_numbers[0]}: {config_line_numbers[1:]}\n')
